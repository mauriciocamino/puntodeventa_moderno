				...
				<table class="table table-bordered table-condensed">
					...
					<tfoot>
						...
						<tr ng-repeat="payments in order.payments" class="{{ payments.type=='return' ? 'danger' : 'success' }}">
							<th ng-show="payments.type=='due_paid'" class="text-right w-60" colspan="2"><small><i></i></small> {{ payments.created_at }} <small><i> por {{ payments.by }}</i></small></th>
							<td ng-show="payments.type=='due_paid'" class="text-right w-40">{{ payments.amount | formatDecimal:2 }}</td>

							<th ng-show="payments.type=='sell'" class="text-right w-60" colspan="2"><small><i>Pagado con</i></small> {{ payments.name }} <i>el</i> {{ payments.created_at }} <small><i>por {{ payments.by }}</i></small></th>
							<td ng-show="payments.type=='sell'" class="text-right w-40">{{ payments.amount | formatDecimal:2 }}</td>

							<th ng-show="payments.type=='return'" class="text-right w-60" colspan="2"><small><i>Devuleto </i></small> {{ payments.created_at }} <small><i> por {{ payments.by }}</i></small></th>
						...
						<tr ng-repeat="payments in order.payments" class="{{ 'success' }}">
							<th ng-show="payments.type=='change'" class="text-right w-60" colspan="2"><small><i>Cambio </i></small> {{ payments.created_at }} <small><i>por {{ payments.by }}</i></small></th>
