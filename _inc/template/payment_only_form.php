				...
				<table class="table table-bordered table-striped table-condensed">
					...
					<tfoot>
						...
						<tr ng-repeat="payments in order.payments" class="{{ payments.type=='return' ? 'danger' : 'success' }}">
							...
							<th ng-show="payments.type=='sell'" class="text-right w-60" colspan="2"><small><i></i></small> {{ payments.name }} <i>El</i> {{ payments.created_at }} <!--<small><i>by {{ payments.by }}</i></small>--></th>
