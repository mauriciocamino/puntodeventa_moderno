					...
					<tfoot>
						...
						<tr ng-repeat="payments in order.payments" class="{{ payments.type=='return' ? 'danger' : 'success' }}">
							<th ng-show="payments.type=='purchase'" class="text-right w-60" colspan="2">
								<small><i>Pagado por</i></small> {{ payments.name }} <i> El </i> {{ payments.created_at }} <small><i> por {{ payments.by }}</i></small></th>
							<td ng-show="payments.type=='purchase'" class="text-right w-40">{{ payments.amount | formatDecimal:2 }}</td>

							<th ng-show="payments.type=='return'" class="text-right w-60" colspan="2"><small><i>El retorno el</i></small> {{ payments.created_at }} <small><i>por {{ payments.by }}</i></small></th>
							<td ng-show="payments.type=='return'" class="text-right w-40">{{ payments.amount | formatDecimal:2 }}</td>

							<th ng-show="payments.type=='change'" class="text-right w-60" colspan="2"><small><i>Equilibrio el</i></small> {{ payments.created_at }} <small><i>por {{ payments.by }}</i></small></th>
