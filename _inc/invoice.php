...
if($request->server['REQUEST_METHOD'] == 'POST' && $request->post['action_type'] == 'DELETE')
{
    try {
        ...
        if (($account_id = store('deposit_account_id')) && $withdraw_amount > 0) {
          ...
          $title = 'Débito al eliminar la factura de venta';
          $details = 'Nombre del cliente: ' . get_the_customer($selling_info['customer_id'], 'customer_name');
          $image = 'NULL';
...
$columns = array(
    ...
    array(
        ...
        'formatter' => function($d, $row) {
            if ($row['is_installment']) {
                return '<span class="label label-warning">Entrega</span>';
            }
    ...
    array( 'db' => 'dian_json', 'dt' => 'dian_json' ),
        array(
        'db' => 'dian_estado',
        'dt' => 'pdf_dian',
        'formatter' => function($d, $row){
        ...
        if ($row['dian_estado'] == 'ok') {
         $html .=  '<a href="https://api.dataico.com/direct/serve-file?document-id='.$row['dian_uuid'].'&amp;pdf-type=pos-invoice" class="btn btn-primary btn-xs fa fa-file-pdf-o isTooltip" target="_blank" style="color: red; font-size: 25px;  background-color: #fff;  border-color: #fff;" title="PDF FACTURA"></a>';
       }

        if ($row['dian_estado_nc'] == 'error') {
          $html .=  '<a href="#" num-noc="'.$row['dian_uuid']."==".$row['invoice_id'].'" class="btn btn-info btn-xs TransmitirNotaCredito  glyphicon glyphicon-export" title="Emitir Nota Credito"></a>';
        }

        if (!empty($row['dian_uuid_nc'])) {      
          $notas = explode('[/]', $row['dian_uuid_nc']);
          foreach ($notas as $k => $n) {
            $html .= '<a href="https://api.dataico.com/direct/serve-file?document-id='.$n.'&amp;pdf-type=credit-note" class="btn btn-primary btn-xs fa fa-file-pdf-o isTooltip" target="_blank" style="color: blue; font-size: 25px;  background-color: #fff;  border-color: #fff;" title="PDF NOTA CREDITO"></a>';
          }
        }
        return $html;
