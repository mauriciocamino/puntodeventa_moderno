...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'RETURN')
{
  try {
    ...
    $statement = db()->prepare("INSERT INTO `returns` SET `store_id` = ?, `reference_no` = ?, `invoice_id` = ?, `customer_id` = ?, `note` = ?, `total_item` = ?, `total_quantity` = ?, `subtotal` = ?, `total_amount` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `total_purchase_price` = ?, `profit` = ?, `created_by` = ?, created_at = ?, updated_at = ?");
    $statement->execute(array($store_id, $reference_no, $invoice_id, $customer_id, $note, $total_item, $total_quantity, $tsubtotal, $tpayable, $titem_tax, $tcgst, $tsgst, $tigst, $total_purchase_price, $profit, $user_id,date_time(),date_time()));
    ...
    if (($account_id = store('deposit_account_id')) && $return_amount > 0) 
    {
      ...
      $title = 'Débito por devolución de producto';
      $details = 'Nombre del cliente: ' . get_the_customer($customer_id, 'customer_name');
      $image = 'NULL';
    ...
    if ($mat_store['fac_electronica'] == 'S') {
      ...
      $statement->execute(array($json,$uuid,$estado,$store_id,$invoice_id));
    }
    ...
    echo json_encode(array('msg' => trans('text_return_success').$msn, 'id' => $item_id));
    exit();
