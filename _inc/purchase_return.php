...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == 'RETURN')
{
  try {
    ...
    $statement = db()->prepare("INSERT INTO `purchase_returns` SET `store_id` = ?, `reference_no` = ?, `invoice_id` = ?, `sup_id` = ?, `note` = ?, `total_item` = ?, `total_quantity` = ?, `subtotal` = ?, `total_amount` = ?, `item_tax` = ?, `cgst` = ?, `sgst` = ?, `igst` = ?, `created_by` = ?, `created_at` = ?, `updated_at` = ?");
    $statement->execute(array($store_id, $reference_no, $invoice_id, $sup_id, $note, $total_item, $total_quantity, $tsubtotal, $tpayable, $titem_tax, $tcgst, $tsgst, $tigst, $user_id,date_time(),date_time()));
    ...
    if (($account_id = store('deposit_account_id')) && $return_amount > 0) 
    {
      ...
      $details = 'Proveedor: ' . get_the_supplier($sup_id, 'sup_id');
      $image = 'NULL';
