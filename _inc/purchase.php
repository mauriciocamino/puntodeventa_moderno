...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'CREATE')
{
  try {
    ...
    $statement->execute(array($sup_id, $reference_no, 'purchase', $pmethod_id, 'Pagado al comprar', $paid_amount, $store_id, $invoice_id, $user_id, $created_at));
    ...
    if ($due > 0) {
        ...
        $statement->execute(array($sup_id, $reference_no, 'due', 'Vencida mientras compra', $due, $store_id, $invoice_id, $user_id, $created_at));
    ...
    if (($account_id = store('deposit_account_id')) && $paid_amount > 0) {
      ...
      $details = 'Proveedor: ' . get_the_supplier($sup_id, 'sup_name');
      $image = 'NULL';
...
if($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE') 
{
  try {
    ...
    $purchase_date = date('Y-m-d H:i:s',strtotime($invoice_info['created_at']));
    ...
    if (($account_id = store('deposit_account_id')) && $deposit_amount > 0) {
      ...
      $title = 'Depósito para compra eliminar';
      $details = 'Proveedor: ' . get_the_supplier($invoice_info['sup_id'], 'sup_name');
      $image = 'NULL';
