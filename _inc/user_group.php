...
$Hooks->do_action('Before_Showing_Usergroup_List');
    ...
    if (user_group_id() == 1) {
      ...
      $table = '(SELECT * FROM user_group WHERE group_id != 1)as user_group';
    }
...
$columns = array(
  ...
  array(
      ...
      'dt' => 'btn_edit',
      'formatter' => function( $d, $row ) {
        if ( user_group_id() != 1 && $row['group_id'] == 2  || $row['group_id'] == 1) {          
  ...
  array(
      ...
      'dt' => 'btn_delete',
      'formatter' => function( $d, $row ) {
        if ($row['group_id'] == 1 || $row['group_id'] == 2 || user_group_id() == $row['group_id']) {          
