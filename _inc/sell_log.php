...
$columns = array(
  ...
  array( 
    ...
    'formatter' => function($d, $row) {
      if ($row['type'] == 'due') {
        return '<span class="label label-danger">'.str_replace('_', ' ', trans_spa(ucfirst($row['type']))).'</span>';
      } elseif ($row['type'] == 'due_paid') {
        return '<span class="label label-success">'.str_replace('_', ' ', trans_spa(ucfirst($row['type']))).'</span>';
      } else {
        return '<span class="label label-warning">'.str_replace('_', ' ', trans_spa(ucfirst($row['type']))).'</span>';
      }
    }
  ),
  array( 
  ...
  array( 
    ...
    'dt' => 'pmethod',
    'formatter' => function($d, $row) {
      return trans_spa(get_the_pmethod($row['pmethod_id'], 'name'));
    }
  ),
  array( 
    'db' => 'amount',   
