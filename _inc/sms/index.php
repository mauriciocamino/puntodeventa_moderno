...
if ($request->server['REQUEST_METHOD'] == 'POST' && $action_type == 'SENDGROUP') 
{
  ...
  $cels = '';
		...
		if (strlen($mobile_number) != 10 ) {
			...
			$response = EnviarMsm($mobile_number, $cmessage);
			$estado_sms = $response == 'MensajeEnviadoCorrectamente' ? 'Enviado' : 'error';
		...
		if (empty($data)) {
			header('Content-Type: application/json');
			echo json_encode(array('msg' => "no hay datos para enviar."));
			exit();
		}

		header('Content-Type: application/json');
		echo json_encode(array('msg' => trans('text_success_sms_sent')." \n Error en los siguientes numeros \n ".$cels));
		exit();
...
if ($request->server['REQUEST_METHOD'] == 'POST' && $action_type == 'SENDINDIVIDUAL') 
{
	try {
		...
		if (strlen($phone_number) != 10 ) {
			throw new Exception("El numero celular debe de taner 10 digitos ".$phone_number);
		}


			$response = EnviarMsm($phone_number, $message);
...
if ($request->server['REQUEST_METHOD'] == 'POST' && $action_type == 'RESEND') 
{
	try {
		...
		//$gw = (new SMSGateway())->initGateway($gateway, sms_setting($gateway));
		//$response = $gw->send($mobile_number, $message);
$response = EnviarMsm($mobile_number, $message);


		$response_array = explode('||',$message);
		if (empty($response_array)) {
			throw new Exception(trans('error_sms_not_send'));
		}
...
{
	$invoice_model = registry()->get('loader')->model('invoice');
	$invoice = $invoice_model->getInvoiceInfo($request->get['invoice_id']);
	$tags = '';
	$statement = db()->prepare("SELECT * FROM `mail_sms_tag` WHERE `type` = ?");
	$statement->execute(array('invoice'));
	$array = $statement->fetchAll(\PDO::FETCH_ASSOC);
...
function EnviarMsm($celulares,$mensaje){
$fecha = date("Y/m/d");

	$mensaje = preg_replace(array('/\s{2,}/', '/[\t\n]/'), ' ', $mensaje);
	$url='http://avisel.com/Avisel.ashx?';
	$data=http_build_query(array(
		"claveapi"      =>"wNwNDDud8QgiO+qnrUIL4LMqSyH4uosoOcUJX6PDz559yIvZSthWdi72ojJVLhlvaNgoVwocUZEVDlrNXMqDMA==",
		"operacion"     =>"enviar",
		"destinatario"  =>$celulares,
		"contenido"     =>$mensaje,
		"fechaEnvio"    =>$fecha,
		"ruta"          => 0
	));
