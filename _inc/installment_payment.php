...
$columns = array(
    ...
    array(
        ...
        'dt' => 'payment_status',
        'formatter' => function( $d, $row) {
            return $row['payment_status'] == 'paid' ? '<span class="label label-success">'.trans_spa(ucfirst($row['payment_status'])).'</span>' : '<span class="label label-danger">'.trans_spa(ucfirst($row['payment_status'])).'</span>';
        }
    ),
    array(
        'db' => 'invoice_id',
