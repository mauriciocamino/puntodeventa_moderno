...
function get_reference_format($sequence)
{
    ...
    switch ($format) {
        ...
        case 'resolucion':
        $store_id = store_id();
        $statement = registry()->get('db')->prepare("SELECT consecutive FROM `invoice_resolution` WHERE `store_id` = ?");
        $statement->execute(array($store_id));
        $sequence = $statement->fetch(PDO::FETCH_ASSOC)["consecutive"];        
        return $sequence+1;
        break;
        case 'random':
        return unique_id(8);
...
function generate_invoice_id($type = 'sell', $invoice_id = null)
{
    ...
    if ($format == 'resolucion') {
        $sequence_format = get_reference_format(0);    
        $invoice_id = $prefix.$sequence_format;
        $consecutive = $prefix.$sequence_format != $invoice_id ? $prefix.$sequence_format : $invoice_id ; 
        $invoice_id = $consecutive;

    }elseif ($invoice_model->hasInvoice($invoice_id)) {

        $invoice_id = str_replace(array('A','B','C','D','E','F','G','H','I','G','K','L','M','N','O','P','Q','R','S','T','U','V','W','X','Y','Z'), '', $invoice_id);
        $sequence = (string)(substr($invoice_id,-8)) + 1;
...
function get_postemplate_data($invoice_id)
{
    ...
    $data = array(
      'logo' => '<img src="'.root_url().'/assets/template/img/logo-favicons/'.store('logo').'">',
      'logo_url' => root_url().'/assets/template/img/logo-favicons/'.store('logo'),
      ...
      'customer_document' => $invoice_info['document'],
      ...
      'order_tax' => currency_format($invoice_info['item_tax']),
      ...
      foreach ($item as $key => $val) {
        if (in_array($key, array('sl'))) {
          $new_item[$key] = $val;
            ...
            $new_item[$key] = currency_format($val * $item['item_quantity']);
