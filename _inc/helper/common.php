...
{
	return trans_date(date("j M Y g:i A", strtotime($date)));
}

function format_only_date($date) 
{
	return trans_date(date("j M Y", strtotime($date)));
}
...
function play_sound($name, $path = null) {
	$path = $path ? $path : root_url() . '/assets/template/mp3/' . $name;
	?>
...
function updateImageValue(&$image, $key) {
  if($key == 'p_image') {
    if (FILEMANAGERPATH && is_file(FILEMANAGERPATH.$image) && file_exists(FILEMANAGERPATH.$image))  {
    	...
    	$image = root_url().'/assets/template/img/noproduct.png';
    }
  }
}
