...
class ModelCron extends Model
{
  ...
  public function CheckForUpdate($action)
  {
    if ($this->IsUpdateAvailable()) {
      $this->msg[] = $action . ': Actualización disponible';
    } else {
  ...
  public function PushSqlToRemoteServer($action)
  {
    if (!checkInternetConnection()) 
    {
      $this->err[] = $action . ': Problema de conexión a Internet';
      return false;
    }

    if (!SYNCSERVERURL || !SYNCHRONIZATION) 
    {
      $this->err[] = $action . ': La sincronización fue deshabilitada desde el archivo config.php';
      return false;
    }
    ...
    $this->msg[] = $action . ': Sincronización realizada con éxito!';
    return true;
  }
