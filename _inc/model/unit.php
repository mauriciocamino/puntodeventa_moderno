...
class ModelUnit extends Model 
{
	...
	{		
    	$statement = $this->db->prepare("INSERT INTO `units` (unit_name, code_name, unit_details, discount) VALUES (?, ?, ?, ?)");
    	$statement->execute(array($data['unit_name'], $data['code_name'], $data['unit_details'], $data['discount']));
	...
	{
    	$statement = $this->db->prepare("UPDATE `units` SET `unit_name` = ?, `code_name` = ?, `unit_details` = ?, `discount` = ? WHERE unit_id = ? ");
    	$statement->execute(array($data['unit_name'], $data['code_name'], $data['unit_details'], $data['discount'], $unit_id));
