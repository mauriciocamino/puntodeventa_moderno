...
class ModelInvoice extends Model
{
    ...
    public function createInvoice($request, $store_id = null)
    {
        ...
        if (INSTALLMENT && $is_installment_order) {
            ...
            $installment_count = (int)ceil($request->post['installment_count']);
        ...
        $details = '';// isset($request->post['payment_details']) ? $request->post['payment_details']['Metodo_Pago'] :'';
        //$details     = serialize($details_raw);
        $details_id  = isset($request->post['payment_details_id']) ? $request->post['payment_details_id']['id_nota'] : '';      
        ...
        foreach ($product_items as $product) 
        {
            ...
            if ($product_info['taxrate']) {
                $taxrate = $product_info['taxrate']['taxrate'];
                if ($product_info['tax_method'] == 'exclusive')
                    $product_info['sell_price'] = ($taxrate / 100 * $product_info['sell_price'])+$product_info['sell_price'];   

                $tax =  ($product_info['sell_price'] * $product_quantity)/($taxrate / 100 +1)*($taxrate / 100 );
            }
        ...
        if ($format == 'resolucion') {
           $statement = db()->prepare("UPDATE `invoice_resolution` SET `consecutive` = `consecutive`+1 WHERE `store_id` = ?");
           $statement->execute(array( $store_id));
       }      

        if ($paid_amount > 0) {
            $statement = $this->db->prepare("INSERT INTO `payments` (store_id, invoice_id, pmethod_id, capital, amount, details, attachment, note, total_paid, pos_balance, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $statement->execute(array($store_id, $invoice_id, $pmethod_id, $capital, $paid_amount, $details, $details_id, $invoice_note, $total_paid, $balance, $user_id, $created_at));
        ...
        if ($pmethod_code == 'credit' && $paid_amount > 0) {
            ...
            $statement->execute(array($customer_id, $reference_no, 'substract_balance', $pmethod_id, 'Reste mientras vende', $paid_amount, $customer_balance, $store_id, $invoice_id, $user_id, $created_at));
        }
        ...
        $statement->execute(array($customer_id, $reference_no, 'sell', $pmethod_id,  'Pagado al vender', $paid_amount, $store_id, $invoice_id, $user_id, $created_at));
        ...
        if (($account_id = store('deposit_account_id')) && $paid_amount > 0) {
            ...
            $title = 'Depósito para vender';
            $details = 'Cliente: ' . get_the_customer($customer_id, 'customer_name');
            $image = 'NULL';
        ...
        if (INSTALLMENT && $request->post['is_installment_order']) {
            ...
            if ($paid_amount > 0) {
                ...
                $statement->execute(array($store_id, $invoice_id, date_time(), $user_id, 'Pago inicial', $capital, $initInterestAmount, $paid_amount, $paid_amount, 'paid'));
            }
    ...
    function duePaid($data, $store_id = null)
    {
        ...
        if ($discount_amount > 0) {
          ...
          $statement->execute(array('discount', $store_id, $invoice_id, $discount_amount, $details, 'descuento mientras se paga', $discount_amount, $user_id, $created_at));
        ...
        if ($pmethod_code == 'credit' && $paid_amount > 0) {
          ...
          $statement->execute(array($customer_id, $reference_no, 'due_paid', $pmethod_id, 'Restar mientras se paga', $paid_amount, $customer_balance, $store_id, $invoice_id, $user_id, $created_at));
        }
        ...
        if (($account_id = store('deposit_account_id')) && $paid_amount > 0) {
          ...
          $source = $statement->fetch(PDO::FETCH_ASSOC);
          $source_id = $source['source_id'];
          $title = 'Depósito por cobranza debida';
          $details = 'Cliente: ' . get_the_customer($customer_id, 'customer_name');
          $image = 'NULL';
          $deposit_amount = $paid_amount;
          $transaction_type = 'deposit';
        ...
        if ($paid_amount > 0) {
          ...
          $statement = db()->prepare("INSERT INTO `sell_logs` (customer_id, reference_no, type, pmethod_id, description, amount, store_id, ref_invoice_id, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
          $statement->execute(array($customer_id, $reference_no, 'due_paid', $pmethod_id, 'Debido pagado', $paid_amount, $store_id, $invoice_id, $user_id, $created_at));
        }
    ...
    {
        $store_id = $store_id ? $store_id : store_id();
        $statement = $this->db->prepare("SELECT `selling_info`.*, `selling_price`.*, `customers`.`customer_id`, `customers`.`customer_name`, `customers`.`customer_city`, `customers`.`customer_state`, `customers`.`type_document`, `customers`.`customer_address`, `customers`.`type_person`, `customers`.`customer_mobile` AS `mobile_number`,`customers`.`document`, `customers`.`customer_email`,`pmethods`.`name`,`invoice_resolution`.* FROM `selling_info` LEFT JOIN `selling_price` ON `selling_info`.`invoice_id` = `selling_price`.`invoice_id` LEFT JOIN `customers` ON `selling_info`.`customer_id` = `customers`.`customer_id` LEFT JOIN `pmethods` ON `pmethods`.`pmethod_id` = `selling_info`.`pmethod_id`  LEFT JOIN `invoice_resolution` ON `invoice_resolution`.`store_id` = `selling_info`.`store_id` WHERE `selling_info`.`store_id` = ? AND (`selling_info`.`invoice_id` = ? OR (`selling_info`.`customer_id` = ?) AND `selling_info`.`inv_type` = 'sell') ORDER BY `selling_info`.`invoice_id` DESC");
    ...
    public function getInvoiceItemsHTML($invoice_id, $store_id = null)
    {
        ...
        $html .= '<td class="text-center" style="padding:0 2px;">Nombre</td>';
        $html .= '<td class="text-right" style="padding:0 2px;">Venta</td>';
        $html .= '<td class="text-center" style="padding:0 2px;">Cant.</td>';
    ...
    public function getInvoiceItemTaxes($invoice_id, $store_id = null)
    {
      ...
      //  $statement = $this->db->prepare("SELECT SUM(`item_quantity`) as qty, SUM(`tax`) as tax, item_tax, `taxrates`.`taxrate_name`, `taxrates`.`code_name` FROM `selling_item` LEFT JOIN `taxrates` ON (`selling_item`.`taxrate_id` = `taxrates`.`taxrate_id`) WHERE `store_id` = ? AND invoice_id = ? GROUP BY `selling_item`.`taxrate_id`");
        $statement = $this->db->prepare("SELECT SUM(`item_price`) as compra, SUM(`item_price`)-SUM(`item_tax`) as base, SUM(`item_tax`) iva, round(tax)tax
FROM `selling_item` LEFT JOIN `taxrates` ON (`selling_item`.`taxrate_id` = `taxrates`.`taxrate_id`) 
WHERE `store_id` = ? AND invoice_id = ? GROUP BY `selling_item`.`taxrate_id`");
        $statement->execute(array($store_id, $invoice_id));
        return $statement->fetchAll(PDO::FETCH_ASSOC);
    }
