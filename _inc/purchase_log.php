...
$columns = array(
  ...
  array( 
    ...
    'formatter' => function($d, $row) {
      if ($row['type'] == 'due') {
        return '<span class="label label-danger">'.str_replace('_', ' ', ucfirst(trans_spa($row['type']))).'</span>';
      } elseif ($row['type'] == 'due_paid') {
        $state = str_replace('due_paid', 'Pagado', $row['type']);
        return '<span class="label label-success">'.str_replace('_', ' ', ucfirst($state)).'</span>';
      } else {
        return '<span class="label label-warning">'.str_replace('_', ' ', ucfirst(trans_spa($row['type']))).'</span>';
      }
    }
  ),
  array( 
