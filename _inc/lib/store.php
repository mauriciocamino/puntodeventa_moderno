...
class Store 
{
	...
	public function __construct($registry) 
	{
		...
		if (isset($this->session->data['store_id'])) {
			...
			$statement = $this->db->prepare("SELECT `stores`.*,ir.fac_electronica,ir.emitir_vender,ir.auto_token,ir.ambiente_fac,ir.id_fac_electronica FROM `stores` LEFT JOIN `invoice_resolution` as ir ON (`ir`.`store_id` = `stores`.`store_id`)  WHERE `stores`.`store_id` = ?");
	...
	public function setStore($store_id)
	{
		...
		$statement = $this->db->prepare("SELECT s.*,ir.consecutive,ir.since,ir.until,ir.resolution,ir.regime,ir.date_since,ir.ambiente_fac,ir.auto_token,ir.fac_electronica,ir.id_fac_electronica,ir.emitir_vender,ir.regime FROM `stores` as s
		LEFT JOIN `invoice_resolution` as ir ON (`ir`.`store_id` = `s`.`store_id`)  WHERE `s`.`store_id` = ?");
		$statement->execute(array($store_id));
		$this->data = $statement->fetch(PDO::FETCH_ASSOC);
	}
