...
function validate_request_data($request) 
{
  ...
  if ($mat_store['fac_electronica'] == 'S') {
    ...
    if (!validateString($request->post['type_person']))
      throw new Exception('Tipo persona es obligatorio');

    if (!validateString($request->post['customer_mobile']))
      throw new Exception('Numero telefono obligatorio');

    if (!validateString($request->post['customer_email'])) 
      throw new Exception('Email es es obligatorio');

    if (!validateString($request->post['customer_address'])) 
      throw new Exception('Direccion es obligatoria');

    if (!validateString($request->post['customer_state'])) 
      throw new Exception('Departamento es obligatorio');

    if (!validateString($request->post['customer_city'])) 
      throw new Exception('Ciudad es obligatoria');
  }
...
$columns = array(
  ...
  array( 
    ...
    'formatter' => function($d, $row) {
      if ($row['dob']) {
        return trans_date(date("j M Y", strtotime($row['dob'])));
      }
      return '-';
    }
  ),
