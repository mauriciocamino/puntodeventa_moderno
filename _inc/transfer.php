  ...
  try {
    ...
    foreach ($request->post['items'] as $key => $item) 
    {
      ...
      if ($info['payment_status'] == 'due') {
        throw new Exception("Invoice ID: " . $item_info['invoice_id'] . " tiene saldo pendiente. Por favor, pague lo adeudado antes de la transferencia");
      }
      ...
      if ($status == 'complete') {
        ...
        if (($account_id = store('deposit_account_id')) && $payable_amount > 0) {
          ...
          $title = 'Depositar mientras se transfieren acciones';
          $details = '';
          $image = 'NULL';
...
$columns = array(
    ...
    array( 
      ...
      'dt' => 'from_store' ,
      'formatter' => function($d, $row) {
          return store_id() == $row['from_store_id'] ? store_field('name', $row['from_store_id']) : store_field('name', $row['from_store_id']);
      }
    ),
    array( 
      'db' => 'to_store_id',   
      'dt' => 'to_store' ,
      'formatter' => function($d, $row) {
          return store_id() == $row['to_store_id'] ? store_field('name', $row['to_store_id']) : store_field('name', $row['to_store_id']);
      }
    ),
    array( 
      'db' => 'total_item',   
    ...
    array( 
      ...
      'formatter' => function($d, $row) {
        if ($row['status'] == 'pending') {
          return '<button class="btn btn-sm btn-block btn-danger" id="transfer-edit" title="'.trans('button_transfer_edit').'">'.trans_spa($row['status']).' <i class="fa fa-fw fa-edit"></i></button>';
        } elseif ($row['status'] == 'sent') {
          return '<button class="btn btn-sm btn-block btn-warning" id="transfer-edit" title="'.trans('button_transfer_edit').'">'.trans_spa($row['status']).' <i class="fa fa-fw fa-edit"></i></button>';
        } else {
          return '<button class="btn btn-sm btn-block btn-success" title="'.trans('button_transfer_edit').'" disabled>'.trans_spa($row['status']).' <i class="fa fa-fw fa-edit"></i></button>';
        }
      }
    ),
);
