...
function (
    ...
    if (window.store.fac_electronica == 'S') {
    var hideColumsArray = [];
    }else{
        var hideColumsArray = [9];
    }   
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: "Lista de facturas -"+from+" al "+to,
                customize: function ( win ) {
                    $(win.document.body)
                        .css( 'font-size', '10pt' )
                        .append(
                            '<div><b><i>Creado Por_: </i></b></div>'
                        )
                        .prepend(
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Lista de facturas -"+from+" al "+to,
                exportOptions: {
                    columns: [ 0, 1, 2, 3 ]
                }
            },
            ...
            },
            {
                ...
                customize: function (doc) {
                    ...
                    doc.content.splice( 1, 0, {
                        ...
                        alignment: 'center',
                        fontSize: 8,
                        text: 'Impreso el: '+window.formatDate(new Date()),
                    });
                    // Create a footer
    ...
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Enviar listado de facturas", title:"Enviar listado de facturas", html: thehtml});
    });
  ...
  $(document).on("click",".TransmitirFactura", function (e) {
            ...
            type: 'POST',
            dataType:"json", 
            data:  {data:$(this).attr('num-fac')},
            success: function(response){
                 window.swal("¡Respuesta!", response, "success");
    ...
    $(document).on("click",".TransmitirNotaCredito", function (e) {

            $.ajax({
            url: window.baseUrl + '/_inc/emitir_dian.php?type=TransmitirNotaCredito',
            type: 'POST',
            dataType:"json", 
            data:  {data:$(this).attr('num-noc')},
            success: function(response){
                 window.swal("¡Respuesta!", response, "success");
                 setTimeout(function() {window.location.reload();}, 1000);
