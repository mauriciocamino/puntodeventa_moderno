...
function (
    ...
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Listado de clientes", title:"Listado de clientes", html: thehtml});
    });

    $(document).on("change",".customer_state", function(e) {
      var id = $(this).val();
      console.log('paapdddda');
      $.ajax({
        url: window.baseUrl + "/_inc/store.php",
        type: 'POST',
        dataType:"json", 
        data: 'action_type=CITY&id='+id,
        success: function(response){
            $("#customer_city").html('');
            $.each(response,function(i,city) {
                $("#customer_city").append("<option value='"+city.code+"'>"+city.name+"</option>");
            })
        }
    });
  });
