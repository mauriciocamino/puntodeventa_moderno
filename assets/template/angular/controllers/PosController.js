...
function (
        ...
        $this.autocomplete({
            source: function (request, response) {
                ...
                .then(function (data) {
                    if (data.data.length <= 0 ) {
                        window.toastr.error("No se encontraron registros con: "+request.term, "Advertencia!");
                        $("#add_item").val("");
                    }
            ...
            select: function ( event, ui ) {
                var names = ui.item.data.split("|");
                 $scope.addItemToInvoice( names[2]);
            ...
            open: function (event, ui) {
                ...
                if ( Number.isInteger(parseInt($("#add_item").val()))) {
    ...
    $scope.addCustomer = function (customer) {
        if ($scope._isInt(customer)) {
            ...
            window.toastr.error("Oops!, Cliente inválida", "Advertencia!");
        }
    };
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        if ($scope.payable != 0 && ($scope.discountAmount >= $scope.payable)) {
            ...
            window.toastr.error("El monto del descuento debe ser menor que el monto a pagar", "Advertencia!");
        }
        if ($scope.payable != 0 && ($scope.shippingAmount >= $scope.payable)) {
            ...
            window.toastr.error("El monto del envío debe ser menor que el monto a pagar", "Advertencia!");
        }
        ...
        then(function(response) {
            if (response.data.p_id) {
                ...
                if (find) {
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            if ((qty > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) && response.data.p_type != 'service') {
                                ...
                                window.toastr.error("¡Este producto está agotado!", "Advertencia!");
                                return false;
                            }
                            if (window.store.sound_effect == 1) {
                    ...
                    item.quantity = qty;
                    item.discount = response.data.disc;
            ...
            window.onbeforeunload = function() {
              return "Los datos se perderán si abandona la página, ¿está seguro?";
            };
    ...
    $scope.CalculateDiscount = function () {
         var mat_dicount = [];
         var descuento   = 0;
         $(".descuento").each(function(i,j){
           mat_dicount[i] = $(j).text().split('--');                         
         });
     
         $.each(mat_dicount,function(i,datos) {
            if (parseInt(datos[1])>0) {
                descuento += (parseInt(datos[2])*parseInt(datos[0])) * (parseInt(datos[1])/100);
            }                           
        });
      $("#discount-input").val(descuento).change();
    }
                ...
                window._.map($scope.itemArray, function (item) {
                    if (item.id == id) {
                        if (item.quantity > 1) {
                            ...
                            window.toastr.error("La cantidad no puede ser menor que 1", "Advertencia!");
                        }
                    }
                });
            }
    ...
    $scope.removeItemFromInvoice = function (index, id) {
        ...
        if ($scope.isEditMode) {
            if ($scope.itemArray.length <= 1) {
                ...
                window.toastr.error("¡El último elemento no se puede eliminar!", "Advertencia!");
                return false;
            }
        }
    ...
    $scope.clickOnPrintOrder = function () {
        if ($scope.itemArray.length <= 0) {
            ...
            window.toastr.error("Por favor, seleccione al menos un artículo de producto", "Advertencia!");
            return false;
        }
    ...
    $scope.payNow = function() {
        ...
        if (!$scope.customerName) {
            ...
            window.toastr.error("Por favor, seleccione una cliente", "Advertencia!");
            return false;
        } 
    ...
    $(document).delegate(".item_quantity", "keyup", function(e) {
        ...
        window._.map($scope.itemArray, function (item) {
            ...
            setTimeout(function() {  $scope.CalculateDiscount();}, 200);
        });
    ...
    $.contextMenu.types.label = function(item, opt, root) {
        ...
        +    "<button class=\"btn btn-default add\" type=\"button\">Agregar</button>"
        +  "</span>"
        +  "</div>")
        .appendTo(this)
        .on("click", "button", function() {
            ...
            if (!itemQuantity || parseFloat(itemQuantity) <= 0) {
                ...
                window.toastr.error("La cantidad debe ser mayor que 0!", "Advertencia!");
                return false;
            }
    ...
    $("#item-list").contextMenu({
        ...
        items: {
            "add": {name: "Agregar 1 (un) artículo", icon: "fa-plus"},
            "sep1": "---------",
            "add_specific_amount": {name: "Cantidad específica", icon: "fa-th", disabled: true},
            "quantity": {
                type: "label", 
            ...
            //"view": {name: "Ver", icon: "fa-eye"},
            "sep2": "---------",
            "edit": {name: "Editar", icon: "fa-pencil"}
        }
    });
    ...
    $scope.FocusPro = function() {
        setTimeout(function() { $("#add_item").focus().change();}, 500);
    };
    $scope.FocusPro();
