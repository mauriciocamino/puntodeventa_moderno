...
function (
        ...
        "buttons": [
            {
                ...
                titleAttr: "Print",
                title: "Listado de transferencia-"+from+" al "+to,
                customize: function ( win ) {
                    $(win.document.body)
                        .css( 'font-size', '10pt' )
                        .append(
                            '<div><b><i>Desarrollado y mantenido por:</i></b></div>'
                        )
                        .prepend(
                            '<div class="dt-print-heading"><img class="logo" src="'+baseUrl+'/assets/template/img/logo-favicons/1_logo.png"/><h2 class="title">'+window.store.name+'</h2><p><b><i>From</i> '+window.from +' <i>To</i> '+window.to+'</b><br>Impreso el: '+window.formatDate(new Date())+'</p></div>'
            ...
            },
            {
                ...
                titleAttr: "Copy",
                title: window.store.name + " > Listado de transferencia-"+from+" al "+to,
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5 ]
                }
            },
    ...
    $('#from_store_id').on('select2:select', function (e) {
        ...
        if (toStoreID == fromStoreID) {
            ...
            window.toastr.warning("No puede seleccionar el mismo almacen", "Advertencia!");
            return false;
        } else {
                ...
                if (find) {
                    window._.map($scope.productsArray, function (sitem) {
                        if (sitem.id == response.data.products.id) {
                            if (sitem.quantity <= 0) {
                                window.toastr.warning('Agotado', "Advertencia!");
                                stopProcess = true;
                            } else {
    ...
    $(document).delegate("#reset", "click", function (e) {
        ...
        $("#to_store_id").val("").trigger("change");
        $("#image_thumb img").attr("src", "../assets/template/img/noimage.jpg");
        $("#image").val("");
    ...
    $("#email-btn").on( "click", function (e) {
        ...
        var thehtml = dt.html();
        EmailModal({template: "default", subject: "Enviar Lista", title:"Enviar Lista", html: thehtml});
    });
}]);
