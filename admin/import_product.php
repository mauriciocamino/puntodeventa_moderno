		...
		if(isset($_FILES["filename"]["type"]))
		{
			...
			if (in_array($file_extension, $validextensions)) {
				if ($_FILES["filename"]["error"] > 0) {
					throw new Exception("Código de retorno: " . $_FILES['filename']['error']);
				} else {
			...
			switch ($Name) {
				case 'Product':
					foreach ($Reader as $Row)
					{
						...
						if ($Row[1] == 'NombreProducto' || !$Row[1]) continue;

						$pro_data['product_name'] = $Row[1];
						$pro_data['product_type'] = $Row[2];
						$pro_data['code'] = isset($Row[3]) ? $Row[3] : '';
						$pro_data['hsn_code'] = '';//isset($Row[4]) ? $Row[4] : '';
						$pro_data['barcode_symbology'] = 'code128';//$Row[5];
						$store_code_names = explode(',', $Row[4]);
						if (count($store_code_names) < 1) {
							$store_code_names = array_unique(array($store_code_names));
						}
						$pro_data['category_id'] = get_category_id_by_slug($Row[5]);
						$pro_data['unit_id'] = get_unit_id_by_code($Row[6]);
						$pro_data['taxrate_id'] = get_taxrate_id_by_code($Row[7]);
						$pro_data['tax_method'] = isset($Row[8]) ? $Row[8] : 'inclusive';
						$pro_data['sup_id'] = get_supplier_id_by_code($Row[9]);
						$pro_data['brand_id'] = get_brand_id_by_code($Row[10]);
						$pro_data['box_id'] = get_box_id_by_code($Row[11]);
						$pro_data['alert_quantity'] = isset($Row[12]) ? (float)$Row[12] : 10;
						$pro_data['cost_price'] = isset($Row[13]) ? (float)$Row[13] : 0;
						$pro_data['sell_price'] = isset($Row[14]) ? (float)$Row[14] : 0;
						$pro_data['description'] = isset($Row[15]) ? $Row[15] : '';
						$pro_data['status'] = isset($Row[16]) ? (int)$Row[16] : 1;
						$pro_data['thumbnail'] = isset($Row[17]) ? $Row[17] : '';
						$img_array = isset($Row[18]) ? $Row[18] : array();
						// dd($pro_data);
						...
						foreach ($store_code_names as $store_code) {
							if (!get_store_id_by_code($store_code)) {
								throw new Exception(trans('código de tienda '.$store_code.' No es válido!').' ('.$pro_data['product_name'].'-'.$pro_data['code'].')');
							}
						}
						...
						<div class="box-body">
							...
							<div class="form-group">
								...
								<div class="col-sm-9">
								    ...
								    <a href="../storage/productos.xls" id="download_demo">
